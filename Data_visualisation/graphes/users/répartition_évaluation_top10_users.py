...
data_path = (base_dir / ".." / ".." / ".." / "Data" / "Data preprocessing" / "u.data").resolve()
data = pd.read_csv(
	data_path,
	sep="\t",
	header=None,
...
ratings_count = data.groupby(["user_id", "rating"])["rating"].count().unstack().fillna(0)
...
ax.set_title('Proportions des évaluations par utilisateur TOP 10')
